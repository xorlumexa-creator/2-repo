...
def _run_calculix_solid_tet(mesh, mat_key, force_n=1000, force_dir="z"):
    ...
    Returns None (never raises) if tetrahedralization or the solve fails, so the
    caller can fall back to the shell path without the request failing.
    """
    if not GMSH:
        return None
    ...
    try:
        ...
        subprocess.run(["ccx", "-i", job_name], capture_output=True, text=True,
                        timeout=300, cwd=os.path.dirname(inp_path))

        max_vm, max_disp = _parse_ccx_dat(job_name + ".dat")
...
def _run_calculix_shell(mesh, mat_key, force_n=1000, force_dir="z"):
    ...
    solid-tetrahedral one. Best suited to thin-walled parts.
    """
    if not CALCULIX:
        return None
...
def run_calculix_fem(mesh, mat_key, force_n=1000, force_dir="z"):
    ...
    if GMSH:
        result = _run_calculix_solid_tet(mesh, mat_key, force_n, force_dir)
        if result is not None:
            return result

    return _run_calculix_shell(mesh, mat_key, force_n, force_dir)
...
async def run_fem(
    ...
    try:
        ...
        result = run_calculix_fem(mesh, material, force_n, force_dir)
        if result is None:
            detail = f" [ccx availability check failed: {CALCULIX_ERROR}]" if CALCULIX_ERROR else ""
            return {"fem_result": None,
                    "note": "Neither solid-tet nor shell FEM produced a result "
                            "(CalculiX unavailable, or the solve failed) — the "
                            f"caller should fall back to its own analytical path.{detail}"}
